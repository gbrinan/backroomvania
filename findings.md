...
- 사용자 제공 설정이며 역사 사건 조사 결과가 아니다. 사건 정본은 decisions/0006-ordinary-night.md.
- Trash와 Rhast는 같은 알파벳 조합이지만 의미·암호 규칙·동일인 여부는 사용자 미확정이므로 해답으로 쓰지 않는다.
- 관장이 존재를 부정한 뒤 마지막 말을 묻는 모순은 의도적인 미스터리 단서로 보존한다.
- 반대 관점: 긴 일상 튜토리얼은 공포 진입을 늦출 수 있다. 유물 두 점과 협동 정리 한 번으로 조작 학습을 제한하고, 실패 시 대사를 반복하지 않는 안을 제안한다.

## 긴 중반과 수집가 반전

- 사용자 답변으로 HAster는 황색의 왕 Hastur와 별개 인물임을 확정. Trash=HAster이며 홈즈에게 조종당한 흰 수집가.
- 기존 concept.md의 네 문화 구역·깃털 걸음·실의 표식·인장·먹빛 시야·지름길과, 후속 한국·아시아·남미·아즈텍·도시 공간을 대조하여 v2 구성. 처음 공유 문서에 관한 기존 조사 요약의 작은 실험 원칙을 유지하고 전체 서사 확장을 당장 구현 범위로 오인하지 않음.
- 반대 관점: 긴 중반을 모든 방의 반복 클리어로 채우면 진행이 늘어진다. 새 능력으로 열린 측실과 새로운 증거를 재방문의 보상으로 제안.

## 백룸 강화

- 사용자 요청은 기존 백룸 요소의 강화. 외부 백룸 레벨·설정을 조사하거나 정전으로 채택한 작업이 아니라 기존 박물관을 활용한 창작 확장.
- 반대 관점: 전 공간을 길 잃기 구간으로 만들면 신화 던전의 개성이 희석된다. 구역 사이의 정적·비정상 공간과 문화별 전투를 교차시키는 안으로 분리.

## 첫 게임 구현 조사 (2026-09-13)
